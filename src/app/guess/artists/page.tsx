...
const { Letters, ArtistsChallengeData } = Data;
...
function Page() {
  ...
  const tracks = ArtistsChallengeData[currentLetter];
  const options = tracks.map(track => track.addedBy).sort();

  const stats: Stats = computeStats(Letters, ArtistsChallengeData, answers, options);
